...
export class AngularNeo4jService {
   ...
   */
  connect(url:any, username:any, password:any, encrypted = true) {
  ...
  private processInteger(integer:any) {
  ...
  private processRecord(record:any) {
    ...
    if (record.constructor.name === 'Path') {
      ...
      record.segments = record.segments.map((segment:any) => {
